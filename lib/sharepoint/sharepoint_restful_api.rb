module Sharepoint
  ...
  module SharepointRestfulApi
    ...
    def sharepoint_access_token
      ...
      url = URI('https://' + config['site_url'] + '/' + config['tenant_id'] + '/OAuth/2')
    ...
    def sharepoint_upload_file(access_token, folder_name, file_name)
      config = YAML.load_file(File.join(__dir__, '../../config/sharepoint.yml'))
      url = URI('https://' + config['site_url'] + "/_api/web/GetFolderByServerRelativeUrl('/Shared Documents/□②事務/◎①見積・発注/" + folder_name + "')/Files/Add(url='" + file_name + "', overwrite=true)")
